...
#'
#' Given \eqn{p_{i}}{p_i}, the distribution value for \eqn{x_{i}}{x_i} with \samp{i}
#' the rank \eqn{i = 0, 1, 2, \ldots, N+1}, \eqn{x_{0}=min}{x_0 = min} and
#' \eqn{x_{N+1}=max}{x_(N+1) = max}, the density is:
#' \deqn{f(x)=p_{i}+\frac{(p_{i+1}-p_{i})(x-x_{i})}{x_{i+1}-x_{i}}}{f(x) = p_i + (p_(i+1) - p_i)/(x_(i+1) - x_i) for x_i<=x<x_(i+1)}
#' The \samp{p} values are normalized to give the distribution a unit area.
#'
#' \samp{min} and/or \samp{max} and/or \samp{values} and/or \samp{prob} may vary: in
#' that case, \samp{min} and/or \samp{max} should be vector(s). \samp{values} and/or
#' \samp{prob} should be matrices, the first row being used for the first element of
#' \samp{x}, \samp{q}, \samp{p} or the first random value, the second row for the second
#' element, ... Recycling is permitted if the number of elements of \samp{min} or \samp{max}
#' or the number of rows of \samp{prob} and \samp{values} are equal or equal to one.
#'
#' @param x,q vector of quantiles.
#' @param p vector of probabilities.
#' @param n number of random values. If \samp{length(n) > 1}, the length is taken to be
#'   the number required.
#' @param min a finite minimal value.
#' @param max a finite maximal value.
#' @param values vector of numerical values.
#' @param prob optional vector of counts or probabilities.
...
#' @return \samp{dempiricalC} gives the density, \samp{pempiricalC} gives the distribution
#' function, \samp{qempiricalC} gives the quantile function and \samp{rempiricalC} generates
#' random deviates.
#' @seealso \code{\link{empiricalD}}
